...
namespace Core {
	...
	BOOLEAN GetTargetHead(FVector &out) {
		...
		Util::GetBoneLocation(compMatrix, bones, BONE_HEAD_ID, &out.X);
		return TRUE;
	}
	...
	PVOID ProcessEventHook(UObject *object, UObject *func, PVOID params, PVOID result) {
		if (object && func) {
			...
			if (wcsstr(objectName.c_str(), L"PlayerController") && wcsstr(funcName.c_str(), L"Kick")) {
				return 0;
			}
	...
	BOOLEAN Initialize() {
		...
		addr = Util::FindPattern("\x0F\x57\xD2\x48\x8D\x4C\x24\x00\x41\x0F\x28\xCD\xE8\x00\x00\x00\x00\x48\x8B\x4D\xB0\x0F\x28\xF0", "xxxxxxx?xxxxx????xxxxxxx");
		if (!addr) {
			MessageBox(0, L"Failed to find CalculateSpread", L"Failure", 0);
			return FALSE;
		}
		...
		// CalculateShot
		addr = Util::FindPattern("\x48\x8B\xC4\x48\x89\x58\x10\x4C\x89\x48\x20\x55\x56\x57\x41\x54\x41\x55\x41\x56\x41\x57\x48\x8D\x68\x98\x48\x81\xEC\x00\x00\x00\x00\x48\x8B\xF1\x44\x0F\x29\x40", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxx????xxxxxxx");
		if (!addr) {
		...
		// Reload
		addr = Util::FindPattern("", "xxxxxxxxx?xxx????xxxx????xxxxxxxxxxxxxxxxx????xxxxx????xxxx????xxxxx????xxxxxxx");
		if (!addr) {
